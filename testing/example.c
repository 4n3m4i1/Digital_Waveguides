...
int main(int argc, char **argv){
    ...
    Pre_Run_Cleanup(&Control);
    printf("Cleaning Up...\n");

    // Create the delay line on the delay_0 block, 50 samples deep
    Init_Delay_Line(Control.blocks[delay_0], 500);
    // Create WAVE output module
    Init_WAV_Output(Control.blocks[WAVE_out], Control.runtime, Control.clk_hz);
    // Create Sine wave source, Fs = clkHz, freq = 500hz, 0 deg phase shift
    Init_Sine_OSC(Control.blocks[SIN_OSC], Control.clk_hz, 500, 0);
    Init_Sine_OSC(Control.blocks[SIN_OSC2], Control.clk_hz, 1500, 90);
    // Initiate an adder
    Init_Simple_Math_Block(Control.blocks[Adder], B_ADD);
    Init_Simple_Math_Block(Control.blocks[Adder2], B_ADD);
    printf("All Blocks Initiated.\n");


    // Wire Modules Together
    // Bi-directional wires
    Wire_Blocks(&Control, &Control.blocks[SIN_OSC]->d_out_0,    &Control.blocks[delay_0]->d_in_0);
    ...
    for(uint64_t n = 0; n < Control.samples_2_run * 2; ++n){
        //printf("CLK %c @ tick %u\n", (clk) ? '1' : '0', n);
        WGB_Sine_OSC(Control.blocks[SIN_OSC], clk);                 //
        WGB_Sine_OSC(Control.blocks[SIN_OSC2], clk);                //
